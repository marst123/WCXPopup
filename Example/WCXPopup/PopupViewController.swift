...
class PopupViewController: UIViewController, MTPopupDelegate {
    ...
    override func viewDidLoad() {
        ...
        button_bottom.setAction { sender in
            ...
            MTPopupManager.shared.showPopup(type: .bottom, hideType: .slideToBottom, contentView: bottomView, identifier: "VIEW_BOTTOM", delegate: self, isTapGestureMaskView: true)
        }
        
        button_top.setAction { sender in
            let bottomView = UIView()
            MTPopupManager.shared.showPopup(type: .top, hideType: .slideToTop, contentView: bottomView, identifier: "VIEW_TOP", delegate: self, isTapGestureMaskView: true)
        }
        
        button_left.setAction { sender in
            let bottomView = UIView()
            MTPopupManager.shared.showPopup(type: .cut(leftPaading: 0, rightPaading: 80), showType: .slideFromLeft, hideType: .slideToLeft, contentView: bottomView, identifier: "VIEW_LEFT", delegate: self, isTapGestureMaskView: true)
        }
        
        button_right.setAction { sender in
            let bottomView = UIView()
            MTPopupManager.shared.showPopup(type: .cut(leftPaading: 80, rightPaading: 0), showType: .slideFromRight, hideType: .slideToRight, contentView: bottomView, identifier: "VIEW_RIGHT", delegate: self, isTapGestureMaskView: true)
        }
        
        button_center.setAction { sender in
            let bottomView = UIView()
            MTPopupManager.shared.showPopup(type: .center(paading: 50), showType: .bounce, contentView: bottomView, identifier: "VIEW_CENTER", delegate: self, isTapGestureMaskView: true)
        }
    }
